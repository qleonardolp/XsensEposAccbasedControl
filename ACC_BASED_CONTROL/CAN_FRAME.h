...
public:
	...
	void initCANFrame(int tipo, char *l_pSelectedInterface, char *l_pSelectedDatabase, char *l_pSelectedCluster, char *l_pSelectedFrame)
	{
			...
			if (type == 1) {
				g_Status = nxCreateSession (l_pSelectedDatabase, l_pSelectedCluster, l_pFrameArray, l_pSelectedInterface, nxMode_FrameOutSinglePoint, &g_SessionRef);
			}
			else{
				g_Status = nxCreateSession (l_pSelectedDatabase, l_pSelectedCluster, l_pFrameArray, l_pSelectedInterface, nxMode_FrameInSinglePoint, &g_SessionRef);
			}
	}
	...
	void write(u8 payload[8], int payload_len)
	{
		...
		if(0 != g_Status)
		{
			PrintStat(g_Status,"nxWriteFrame");  
		}
	}
